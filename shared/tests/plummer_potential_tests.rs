...
use shared::potentials::Potential;
use shared::potentials::plummer::PlummerPotential;
...
fn magnitude_increases_then_decreases_around_peak() {
    ...
    for r in below {
        ...
        assert!(mag > last, "expected |F| to increase for r<r_peak; r={r}, |F|={mag}, last={last}");
        last = mag;
    }
    ...
    for r in above {
        ...
        let (fx, fy, fz) = pot.force(0.0, x, y, z);
        let mag = norm(fx, fy, fz);
        assert!(mag < last, "expected |F| to decrease for r>r_peak; r={r}, |F|={mag}, last={last}");
        last = mag;
    }
}
