...
| 10 | Compile with zero warnings at the strictest settings | `lefthook` gate: `cargo clippy --workspace --all-targets -- -D warnings` with `clippy::pedantic` enabled ([workspace.lints.clippy]); the Python surface is gated by `basedpyright` (zero diagnostics). |
