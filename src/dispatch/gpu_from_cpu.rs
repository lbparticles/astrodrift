...
//     let kernel = py_runtime_err(module.get_function("dopr54_cpu_port"))?;
...
pub fn gpu_dispatch2(
    ...
    let dump = parse_dopr54_dump("dopr54_init_dump.txt")?;
    ...
    let kernel = py_runtime_err(module.get_function("dopr54_cpu_port"))
        .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("{e}")))?;
    let dev_state0 = py_runtime_err(DeviceBuffer::<f64>::from_slice(&state0))
        .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("{e}")))?;
    let dev_times = py_runtime_err(DeviceBuffer::<f64>::from_slice(&times))
        .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("{e}")))?;
    let dev_state_out =
        py_runtime_err(DeviceBuffer::<f64>::from_slice(&state_out))
        .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("{e}")))?;
    ...
    let mut f = File::create("dopr54_rust_out_gpu.txt")?;
    let err: c_int = 0;
    writeln!(f, "err {}", err)?;
