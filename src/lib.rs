...
fn simulation_ctx<'py>(
    ...
    let target_t_end = if config.reverse { -config.t_end } else { config.t_end };
    ...
    let mut dt_host = vec![time_direction * config.dt0; n];
    ...
    let w0 = w_host[0] as usize;
    let _traj_len = (w0 + 1).min(config.steps_cap);
