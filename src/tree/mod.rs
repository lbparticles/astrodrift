...
impl AdjacencyMatrix {
    ...
    /// Row `r` as an 11-bit mask. Fits in `u16` because it is masked to `Self::N` bits.
    #[allow(clippy::cast_possible_truncation)] // masked to Self::N = 11 bits
    fn row_mask(&self, r: usize) -> u16 {
        (self.row_bits(r) & 0x7FF) as u16
    }
    ...
    #[must_use]
    pub fn last_true_column_power(&self, cap: u8) -> [u8; 11] {
        let n = Self::N;
    ...
    pub fn mul_bool(&self, rhs: &AdjacencyMatrix) -> AdjacencyMatrix {
        ...
        for (j, br) in b_col_rows.iter_mut().enumerate() {
            *br = rhs.column_rows_mask(j);
        }

        let mut out: u128 = 0;
        for (i, &ar) in a_rows.iter().enumerate() {
            for (j, &br) in b_col_rows.iter().enumerate() {
                if ar & br != 0 {
                    out |= 1u128 << Self::idx(i, j);
                }
            }
    ...
    #[must_use]
    pub fn build(&self, containers: &[Option<Box<Container>>; 11]) -> (Model, InputFrame) {
        ...
        zeros.sort_unstable(); // stable deterministic tail
        ...
        for v in 0..11 {
            ...
            if let Some(c) = containers[v].as_ref() {
                istates_by_stage[s].clone_from(&c.state);
            }
            ...
            if arr_k.iter().any(std::option::Option::is_some) {
