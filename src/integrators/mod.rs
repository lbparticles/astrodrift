...
/// Runs one integration pass over `input_frame` with the engine/method/variant
/// selected in `config` and the potential `model`.
///
/// # Panics
///
/// Panics if the dispatch to the selected backend (`gpu_dispatch` or
/// `cpu_dispatch`) fails.
#[must_use]
pub fn run_integration(config: &Config, model: &Model, input_frame: &InputFrame) -> OutputFrame {
    match (config.engine, config.method, config.variant) {
        ...
        _ => OutputFrame(core::array::from_fn(|_| None)),
    }
}
