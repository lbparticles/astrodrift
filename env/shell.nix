# Native NixOS development environment for astrodrift (MODERN flavor).
#
# Faithful port of env/Dockerfile.modern + .devcontainer/devcontainer.json
# to a nix-shell, minus Docker itself. The default kernel path is
# cuda-oxide (Rust -> LLVM IR -> PTX/cubin), so unlike the legacy shell we
# do NOT build LLVM 7: that exists only for the Rust-CUDA
# `rustc_codegen_nvvm` backend. For legacy builds (astrodrift-0.1.0+legacy)
# use env/shell-legacy.nix + scripts/build-legacy.sh instead.
#
# Usage:
#   nix-shell                 # from the repo root (wrapper) or:
#   nix-shell env/shell.nix --run '<cmd>'
#
...
pkgs.mkShell {
  ...
  shellHook = ''
    ...
    # GPU target for ./build-cuda-oxide-kernels.sh. sm_80 is the project
    # default (matches the container); override when building for a
    # different GPU.
    export CUDA_OXIDE_ARCH="''${CUDA_OXIDE_ARCH:-sm_80}"
